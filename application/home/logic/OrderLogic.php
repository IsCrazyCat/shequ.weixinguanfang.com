...
{
    public function addPiFa($user_id,$jianmone){

        $cartList = M('Cart')->where(['user_id'=>$user_id,'selected'=>1])->select();

        foreach($cartList as $key => $val) {
            $data9['user_id'] = $user_id; // 购买人id
            $data9['goods_id'] = $val['goods_id']; // 商品idd
            $data9['goods_name'] = $val['goods_name']; // 商品名称
            $data9['goods_num'] = $val['goods_num']; // 购买数量
            $data9['member_goods_price'] = $val['member_goods_price']; // 会员折扣价
            $data9['addtime'] = time(); //

            $daishou_goods_id = M("daishou_goods")->insertGetId($data9);
            $daishou_goods_id = M("daishou_goods")->insertGetId($data9);
        }

        //抵扣批发券和等值的余额
        if($jianmone>0){
            M('Users')->where("user_id", $user_id)->setDec('pfj_money',$jianmone);
            //減少库存
            M('goods')->where(array('goods_id'=>32))->setDec('store_count',$val['goods_num']);
            /*2019-08-31
            //抢购间隔（更新抢购时间）
            $dqq['qianggou_time']=time();
            M('Users')->where(array('user_id'=>$user_id))->update($dqq);
            //抢购间隔 end
            */
        }
        // 4 删除已提交订单商品
        ...
        $data4['pifajuan'] = -$jianmone;
//        $data4['user_money'] = -$jianmone;
        $data4['change_time'] = time();
        $data4['desc'] = '消费批发券和等值的余额';
        $data4['jxmc'] = '购买批发区商品';
        // 如果使用了积分或者余额才记录
         M("AccountLog")->add($data4);

        return array('status'=>3,'msg'=>'提交订单成功');
    }
    /**
     * 添加一个订单
     * @param $user_id|用户id
...
//        {
            update_pay_statusSY($order['order_sn']);
//        }
...
if($pay_name=="批发券支付"){
    M('Users')->where("user_id", $user_id)->setDec('pfj_money',$car_price['balance']); // 抵扣批发券
   // M('Users')->where("user_id", $user_id)->setDec('user_money',$car_price['balance']); // 抵扣批发券
}else{
    if($car_price['balance']>0)
        ...
        $data4['user_id'] = $user_id;
        if($pay_name=="批发券支付"){
            $data4['pifajuan'] = -$car_price['balance'];
           // $data4['user_money'] = -$car_price['balance'];
            $data4['desc'] = '批发券购物消费（发货）';
        }else{
            $data4['user_money'] = -$car_price['balance'];
            $data4['desc'] = '购物消费';
        }
        ...
        // 如果使用了积分或者余额才记录
        ($data4['user_money'] || $data4['pay_points'] || $data4['pifajuan']) && M("AccountLog")->add($data4);
