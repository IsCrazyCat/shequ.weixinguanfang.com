...
class Index extends Base {
    ...
    public function qr_code_new()
    {
        ...
        /* 生成二维码 */
        $qr_code_file = $qr_code_path.time().rand(1, 10000).'.png';
        \QRcode::png($url, $qr_code_file, QR_ECLEVEL_H);
        ...
        if ($head_pic) {
            ...
            //如果是本地头像
            if (file_exists(ROOT_PATH.$head_pic)) {
                $logo = Image::open(ROOT_PATH.$head_pic);
                $logo_width = $logo->height();
                $logo_height = $logo->width();
                $logo_qr_width = $QR_width / 5;
                $scale = $logo_width / $logo_qr_width;
...
//            }
        ...
        $file_bg = SHAREQRCODE.'/sybg.jpg';
        $img_bg = imagecreatefromjpeg($file_bg);
        ...
        //二维码
        $width_code = imagesx($qrHandle);
        $height_code = imagesy($qrHandle);
     //   imagecopymerge($im, $qrHandle, ($width-$width_code)/2, $height-$height_code-600, 0, 0, $width_code, $height_code, 100);
        imagecopyresized ($im, $qrHandle, ($width-350)/2, $height-$height_code-650, 0, 0, 350, 350, $width_code, $height_code);

        //左上头像
       $head_pic = imagecreatefromstring(file_get_contents($logo_file));
        $width_head = imagesx($head_pic);
        $height_head = imagesy($head_pic);
        imagecopyresized ($im, $head_pic, 100, 60, 0, 0, 140, 140, $width_head, $height_head);

        //头像下部文字d
        $font_simhei = PUBLIC_FONTS.'/simhei.ttf';
        $text_title = '';
        $color_title = ImageColorAllocate($im, 86, 83, 83);
        $v = imagettftext($im, 22, 0, 82, 240, $color_title, $font_simhei, $text_title);
