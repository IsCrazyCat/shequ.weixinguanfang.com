...
use think\Verify;
use think\Db;
use app\common\util\GenerateQRCode;
...
//		M("users")->where("user_id", $this->user_id)->save($vv);	
    ...
    public function lower_listq(){
        ...
        switch (I('get.type')){
            case "HHR":
                $where['level']=3;
                $count = Db::name('users')->where(" second_leader = {$this->user_id} ")->count();
                $page = new Page($count,C('PAGESIZE'));
                $lists = Db::name('users')
                    ->field('nickname,user_id,distribut_money,reg_time,head_pic,level,first_leader,second_leader,total_amount')
                ...
                foreach($lists as $key => $value){
                    $lists[$key]['yjnum'] = Db::name('users')->where(" second_leader = '". $value['user_id'] ."' ")->count();
                    $shangj=Db::name('users')->where(" user_id = '". $value['first_leader'] ."' ")->find();
                    $lists[$key]['tuiid']=$value['first_leader'];
                    $lists[$key]['tuiname']=$shangj['nickname'];
                }
                ...
                $this->assign('page', $page->show());// 赋值分页输出
                $this->assign('lists',$lists); // 代理
                $this->assign('mycount', $count);
                $this->assign('san', 3);
                $u_xit= Db::name('users')->where(" user_id = {$user['second_leader']} ")->find();
                $this->assign('tuiid', $user['second_leader']);//推荐人id
                $this->assign('mycoddwunt', $u_xit['nickname']);//推荐人名字
                break;
            case "GJHHR":
                $where['level']=3;
                $count = Db::name('users')->where(" third_leader = {$this->user_id} ")->count();
                $page = new Page($count,C('PAGESIZE'));
                $lists = Db::name('users')
                    ->field('nickname,user_id,distribut_money,reg_time,head_pic,level,first_leader,second_leader,total_amount')
                ...
                foreach($lists as $key => $value){
                    $lists[$key]['yjnum'] = Db::name('users')->where(" third_leader = '". $value['user_id'] ."' ")->count();
                    $shangj=Db::name('users')->where(" user_id = '". $value['first_leader'] ."' ")->find();
                    $lists[$key]['tuiid']=$value['first_leader'];
                    $lists[$key]['tuiname']=$shangj['nickname'];
                }
            ...
            default:
                ...
                $count = Db::name('users')->where(" first_leader = {$this->user_id} ")->count();
                $page = new Page($count,C('PAGESIZE'));
                $lists = Db::name('users')
                    ->field('nickname,user_id,distribut_money,reg_time,head_pic,level,first_leader,second_leader,total_amount')
                ...
                $this->assign('lists',$lists); // 代理
                $this->assign('mycount', $count);
                $u_xit= Db::name('users')->where(" user_id = {$user['first_leader']} ")->find();
                $this->assign('tuiid', $user['first_leader']);//推荐人id
                $this->assign('mycoddwunt', $u_xit['nickname']);//推荐人名字
        }

        if ($_GET['is_ajax']) {
            return $this->fetch('ajax_lower_listq');
            exit;
        }
        return $this->fetch();
  }
   ...
   /**
    ...
    /**
     * 个人推广二维码 （我的名片）
     * @author lishibo 20190322
     * 二维码合成
     */
    public function qr_code(){
        //只有消费会员才可以有分享功能
//        if($this->user['level'] < 2 and $this->user['user_id'] != 613){//613为调试账号
//            $this->error("您还没有消费体验产品，不能使用二维码");
//        }
        if($this->user['is_distribut'] == 1){
            ...
            $this->assign('ShareLinkm',"http://{$_SERVER[HTTP_HOST]}/index.php?m=Mobile&c=User&a=reg&first_leader={$this->user_id}");
            $this->assign('head_pic',$this->user['head_pic']);
        }else{
            $this->error("您未获得推广权限，不能使用二维码");
        }
        return $this->fetch();
    }

    public function qr_code_bak(){
    ...
    public function must_see(){
        $article = D('article')->field('article_id,title,content')->where(["cat_id"=>88,'is_open'=>1])->cache(true)->select();
