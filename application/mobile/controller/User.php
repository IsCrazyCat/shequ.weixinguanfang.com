...
class User extends MobileBase
{
    ...
    */
    public function _initialize()
    {
        ...
        /******add by lishibo 20190327 获取全局设置*******/
        $config = tpCache('shop_info');
        $this->assign('tpshop_config', $config);//当前配置项
    }
    ...
    public function fahuo()
    {
        ...
        $ordergengx = M('order')->where(array('order_id' => $order_id))->update(['order_prom_type' => 0]);
        if ($ordergengx) {
            $this->success("选择发货成功，等待发货");
        } else {
            $this->error("操作失败");
        }
    ...
    public function zhuanmai()
    {
        ...
        $user = $logic->get_info($user_id); //当前登录用户信息
        $order_id = I('get.order_id');
        $ordergengx = M('order')->where(array('order_id' => $order_id))->update(['order_prom_type' => 8]);
        $order = M('order')->where(array('order_id' => $order_id))->find();
        $duiying = M('goods')->where("pifajuan=" . $order['goods_price'])->find();
        $yeji = $duiying['shop_price'];//直推人应得业绩值
        if ($ordergengx) {
            $user_m = M('Users')->where("user_id", $user_id)->setInc('pfj_money', 400);
            $zm_m = M('Users')->where("user_id", $user_id)->setInc('user_money', 380);

            if ($user_m && $zm_m) {
                $data4['user_id'] = $user_id;
                $data4['pifajuan'] = +400;
                $data4['user_money'] = +380;
                $data4['change_time'] = time();
                $data4['desc'] = '选择了转卖';
                $data4['order_id'] = $order_id;
                $log = M("AccountLog")->add($data4);
                if ($log) {
                    $u = M('Users')->where("user_id", $user_id)->find();
                    //增加转卖人的复投业绩
                    $xi['yeji'] = $yeji;
                    $xi['fyeji'] = $yeji;
                    $jiexin = M('users')->where("user_id=" . $order['user_id'])->save($xi);
                    if ($jiexin) {
                        $rebq['buy_user_id'] = $order['user_id'];
                        $rebq['nickname'] = $u['nickname'];
                        $rebq['order_sn'] = $order['order_sn'];
                        $rebq['order_id'] = $order['order_id'];
                        $rebq['goods_price'] = $order['goods_price'];
                        $rebq['money'] = $yeji;
                        $rebq['status'] = 3;
                        $rebq['create_time'] = time();
                        $rebq['jxmc'] = "购买批发区商品转卖人获得复投业绩";
                        M('rebate_log')->add($rebq);
                        $logq['user_id'] = $order['user_id'];
                        $logq['yeji'] = $yeji;
                        $logq['change_time'] = time();
                        $logq['desc'] = "购买批发区商品转卖人获得复投业绩";
                        $logq['order_sn'] = $order['order_sn'];
                        $logq['order_id'] = $order['order_id'];
                        $logq['jxmc'] = "转卖人复投业绩";
                        M('account_logq')->add($logq);

                    }
                    //增加转卖人的复投业绩 end
                    if ($u['first_leader'] > 0) {//如果有直推人，给直推人增加业绩和5%奖金

                        $tui = M('Users')->where("user_id", $u['first_leader'])->find();
                        $duiying = M('goods')->where("pifajuan=" . $order['goods_price'])->find();
                        $yeji = $duiying['shop_price'];//直推人应得业绩值
                        $zms_m = M('Users')->where("user_id", $u['first_leader'])->setInc('yeji', $yeji);
                        M('Users')->where("user_id", $u['first_leader'])->setInc('fyeji', $yeji);
                        $fen = 20 / 24;
                        $jj = $yeji * 0.05 * $fen;//直推人应得奖金
                        $jj_m = M('Users')->where("user_id", $u['first_leader'])->setInc('user_money', $jj);
                        if ($zms_m && $jj_m) {
                            $reb_log['user_id'] = $u['first_leader'];
                            $reb_log['buy_user_id'] = $order['user_id'];
                            $reb_log['nickname'] = $u['nickname'];
                            $reb_log['order_sn'] = $order['order_sn'];
                            ...
                            $reb_log['jxmc'] = "批发区商品转卖直推人获得奖金";
                            M('rebate_log')->add($reb_log);
                            $reb_log['money'] = $yeji;
                            $reb_log['jxmc'] = "批发区商品转卖直推人获得复投业绩";
                            M('rebate_log')->add($reb_log);
                            $account_log = array(
                                'user_id' => $u['first_leader'],
                                'user_money' => $jj,
                                'yeji' => $yeji,
                                'change_time' => time(),
                                'desc' => "批发区转卖直推人获得奖金和复投业绩",
...
//奖金明细--财务概况
    public function cwgk()
    {
        ...
        //推荐数
        $tui_con = M('users')->where(array('first_leader' => $user_id))->count();
        $this->assign('tui_con', $tui_con);
        //团队数
        $where = array();
        $u_xia = Db::name('users')->where(" first_leader = {$user_id} ")->select();
        $arr_id = "";//当前层所有的下一层id
        $tuan_id = "";//团id
        while ($u_xia) {
        ...
        $tuan_id = substr($tuan_id, 0, -1);
        $tuan_count = Db::name('users')->where('user_id', 'in', $tuan_id)->where($where)->count();
        $this->assign('tuan_con', $tuan_count);
        return $this->fetch();
    }
    ...
    public function index()
    {
        ...
        $this->assign('user', $user['result']);
        $this->assign('money', $money);
        $_SESSION['keyong'] = $user['result']['pfj_money'];
        $first_nickname = Db::name('users')->where("user_id", $this->user['first_leader'])->getField('nickname');
        $this->assign('first_nickname', $first_nickname);
        //推荐数
        $tui_con = M('users')->where(array('first_leader' => $user_id))->count();
...
//            $cartLogic->login_cart_handle($this->session_id, $res['result']['user_id']);  //用户登录后 需要对购物车 一些操作
    ...
    public function reg()
    {
        ...
        $invite = 0;
        $invite = I('get.first_leader');//推荐人id
        if ($invite) {
            $bu = M('users')->where(array('user_id' => $invite))->find();
            $this->assign('shangm', $bu['nickname']);
        }
        ...
        if (IS_POST) {
            ...
            $mobile_codeq = I('post.mobile_code', '');
            $name = I('post.name', '');
if(empty($name)){
    $data['status'] = 8;
    $data['msg'] = "请输入姓名！";
    ...
    exit;
            ...
            if ($mobile_codeq != session($username)) {
                $data['status'] = 3;
                $data['msg'] = "验证码不正确！";
                $this->ajaxReturn($data);
                exit;
            ...
            if (!empty($tui)) {
                $find = M('users')->where(array('mobile' => $tui))->find();
                if (!$find) {
                    $data['status'] = 3;
                    $data['msg'] = "推荐人不存在！";
                    $this->ajaxReturn($data);
            ...
            $data = $logic->regsyNew($username, $mobile, $password, $password2, $chushengriqi, 0, $invite,$name);
            if ($data['status'] != 1) {
                $this->ajaxReturn($data['msg']);
            }
            $da['shenfenzheng'] = I('post.shenfenzheng', '');
            M('users')->where(array('user_id' => $data['result']['user_id']))->update($da);
...
//          $cartLogic->login_cart_handle($this->session_id, $data['result']['user_id']);  //用户登录后 需要对购物车 一些操作
    ...
    /*代售区*/
    public function daishouqu()
    {
        $where['user_id'] = $this->user_id;
        $count = M('daishou_goods')->where($where)->count();
        $Page = new Page($count, 10);
        $show = $Page->show();
        $order_str = "rec_id DESC";
        $order_list = M('daishou_goods')->order($order_str)->where($where)->limit($Page->firstRow . ',' . $Page->listRows)->select();


        foreach ($order_list as & $va) {
            $ing = M('goods')->where("goods_id=" . $va['goods_id'])->find();
            $va['img'] = $ing['original_img'];
...
//var_dump($order_list);die;
        ...
        if ($_GET['is_ajax']) {
            return $this->fetch('ajax_daishou');
            exit;
        }
        return $this->fetch();
    }
...
//        }
        ...
        //获取系统设置的冷冻时间
        $find = M('config')->where(array('name' => "lengdong"))->find();
        //统计订单商品数量
        foreach ($order_list as $key => $value) {
            ...
            $qds = $find['value'] * 86400 + $order_list[$key]['pay_time'];
            if ($qds - time() < 0) {
                $order_list[$key]['end'] = 2;
            }
            $qds = $qds - (8 * 3600);
            $order_list[$key]['endTime'] = $qds;
            $arr_id = $arr_id . "," . $order_list[$key]['order_id'];
        }
        $arr_id = substr($arr_id, 1);
...
//        $this->success($data['msg']);
    ...
    public function userinfo()
    {
        ...
        if (IS_POST) {
            ...
            I('post.shenfenzheng') ? $post['shenfenzheng'] = I('post.shenfenzheng') : false; //昵称
            I('post.kaihuhang') ? $post['kaihuhang'] = I('post.kaihuhang') : false; //昵称
            I('post.card_no') ? $post['card_no'] = I('post.card_no') : false; //昵称
    ...
    */
    ...
    public function order_confirm()
    {
        ...
        if ($data['status'] != 1) {
            ...
            /************************审核用户等级 20190214*******************/
            $myuser = M('users')->where("user_id", $this->user_id)->find();
            $owner_level = (int)$myuser['level'];

            /*****************************会员资格**********************************/
            $distribut_flag = false;
            $order_goods = M('order_goods')->where("order_id=" . $id)->select();
            foreach ($order_goods as $val) {
                $goods = M('goods')->where("goods_id=" . $val['goods_id'])->find(); // 单个商品的佣金
                $is_level = (int)$goods['is_level'];//商品按分类分组
                if ($is_level > 0) {
                    $distribut_flag = true;
                }
            }
            ...
            if ($distribut_flag) {
                ...
                if ($owner_level < 3) { //非活动产品，审核个人等级  会员
                    M('users')->where("user_id", $this->user_id)->save(array('level' => 3));
                }

                /************************多少天后自动分销记录自动分成 20190214*******************/
                if (file_exists(APP_PATH . 'common/logic/DistributLogicSY.php')) {
                    $distributLogic = new \app\common\logic\DistributLogicSY();
                    $distributLogic->confirm_justs($id); // 分成
                }

            } else {
                if ($owner_level < 2) { //活动产品 顾客购买活动产品，获得体验会员资格
                    M('users')->where("user_id", $this->user_id)->save(array('level' => 2)); //会员
                }
            }

            /********************STEP 2 审核用户等级(所有产品都参与 20190525) **********************/
            if ($myuser['first_leader'] > 0) {
                $first_leader = M('users')->where("user_id", $myuser['first_leader'])->find();
                if ($first_leader) {
                    $model->CheckingRelationshipLevelNew($this->user_id, 1);
                }
            }
    ...
    public function return_goods()
    {
        ...
        $return_str_address = "";
        if ($return_address) {
            foreach ($return_address as $key => $val) {
                $return_str_address = $return_str_address . "" . $val;
            }
        }
        $this->assign('return_address', $return_str_address);
...
//            $this->verifyHandle('withdrawals');
            ...
            $mobile = $data['account_bank'];
            $jieguo = M('users')->where(array('mobile' => $mobile))->find();
            if (!$jieguo) {
                $this->error('当前系统没有该用户！', U('user/huzhuan'));
            }
            $money = $data['money'];

            $jiehsou = M('users')->where(array('user_id' => $jieguo['user_id']))->setInc('user_money', $money);
            if ($jiehsou) {
                $account_log = array(
                    'user_id' => $jieguo['user_id'],
                    'user_money' => +$money,
                    'change_time' => time(),
                    'desc' => '获得转账，转账人id:' . $this->user_id,
                );
                M('account_log')->add($account_log);
                $mos=$money*0.1+$money;
                $jian = M('users')->where(array('user_id' => $this->user_id))->setDec('user_money', $mos);
                if ($jian) {
                    $account_log = array(
                        'user_id' => $this->user_id,
                        'user_money' => -$mos,
                        'change_time' => time(),
                        'desc' => '转出金额，转出给id:' . $money,
                    );
                    $shu = M('account_log')->add($account_log);
                    if ($shu) {
                        $this->success("转账成功！", U('User/index'));
                    } else {
                        $this->error('转账失败！');
                        exit;
                    }
...
//            $this->verifyHandle('withdrawals');
            ...
            if (M('withdrawals')->add($data)) {
                //$this->success("已提交申请");
                $this->success("已提交申请,48小时内打款", U('User/Index'));
                exit;
            } else {
