...
class MobileBase extends Controller {
    ...
    public function _initialize() {
        ...
        if (ACTION_NAME  != "article") {
            //header("location:" . U('Article/article',array('article_id'=>1416)));
            //exit;
        }
        ...
        if(strstr($_SERVER['HTTP_USER_AGENT'],'MicroMessenger')){
            ...
            if (isset($user_temp['user_id']) && $user_temp['user_id']) {
                ...
                if (!$user) {
                    ...
                    if($subscribe==1){
                        M('users')->where("user_id", $user_temp['user_id'])->save(array("is_subscribe"=>1));
                    }
                    session('subscribe',$subscribe);// 当前这个用户是否关注了微信公众号
                    setcookie('subscribe',$user['is_subscribe']);
                }
            }
    ...
    public function get_access_token_bak(){
        //判断是否过了缓存期
