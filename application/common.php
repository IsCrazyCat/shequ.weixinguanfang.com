 ...
 * @return boolean
 */
function update_user_level_sy($user_id){
	...
	$myuser = M('users')->where("user_id", $user_id)->find();//博商网络
    if($level_info){
		...
		$where2['first_leader'] = array('eq',$user_id);//成功培养一名业务经理后
 ...
 */
function minus_stock($order_id,$shiling){
    $orderGoodsArr = M('OrderGoods')->master()->where("order_id", $order_id)->select();
    foreach($orderGoodsArr as $key => $val)
    {
        $duiying=M('goods')->where(array('goods_id'=>$val['goods_id']))->find();
        $countq=M('daishou_goods')->where(array('member_goods_price'=>$duiying['pifajuan'],'status'=>0))->count();
        // 有选择规格的商品
        ...
        {   // 先到规格表里面扣除数量 再重新刷新一个 这件商品的总数量
            ...
            if($countq<$val['goods_num']){
                $shucha=$val['goods_num']-$countq;
            M('Goods')->where("goods_id", $val['goods_id'])->setDec('store_count',$shucha); // 直接扣除商品总数量
            }
        }
        ...
        //qiuhongyangs
if($shiling=2){//如果购买的零售区的商品
        if($countq>$val['goods_num']){
            $pilist=M('daishou_goods')->where(array('member_goods_price'=>$duiying['pifajuan'],'status'=>0))->order('addtime asc')->limit($val['goods_num'])->select();
        }else{
            $pilist=M('daishou_goods')->where(array('member_goods_price'=>$duiying['pifajuan'],'status'=>0))->order('addtime asc')->select();
        }
        foreach ($pilist as $va){
            $shouyi=$val['goods_price']*$va['goods_num']*0.60;//卖的钱
            $jia=M('users')->where("user_id", $va['user_id'])->setInc('user_money',$shouyi);
            $jias=M('users')->where("user_id", $va['user_id'])->setInc('xiaoshoue',$shouyi);
            //上线拿团队业绩
            $uqiu=M('users')->where(array('user_id'=>$va['user_id']))->find();
            $shangUser=M('users')->where(array('user_id'=>$uqiu['first_leader']))->find();
            while ($shangUser){
                if($shangUser['level']!=0 &&  $shangUser['level']!=1){
                switch ($shangUser['level']){
                    case 3://如果是区级代理，拿团队业绩的2%
                        $bai=0.02;
                        break;
                    case 4://如果是市     3%
                        $bai=0.03;
                ...
                $basi=$bai*$shouyi;
                    $jiadd=M('users')->where("user_id", $shangUser['user_id'])->setInc('user_money',$basi);
                    if($jiadd){
                        $dataqq=array(
                            'user_id'=>$shangUser['user_id'],
                            'user_money'=>+$basi,
                            'change_time'=>time(),
                            'desc'=>'拿id:'.$va['user_id'].'的团队业绩',
                        );
                        M("AccountLog")->add($dataqq);
                    }
                }
                $shangUser=M('users')->where(array('user_id'=>$shangUser['first_leader']))->find();
            }
            //上线拿团队业绩 end
            if($jia){
                $dataqq=array(
                ...
                M("AccountLog")->add($dataqq);
//                M('daishou_goods')->save(['status'=>1],['rec_id'=>$va['rec_id']]);
                $bian['status']=1;
                $bian['addtime']=time();
                M('daishou_goods')->where(array('rec_id'=>$va['rec_id']))->update($bian);
            }
...
function accountLog2($user_id, $user_money = 0,$pay_points = 0, $desc = '',$distribut_money = 0,$order_id = 0 ,$order_sn = ''){
    /* 插入帐户变动记录 */
    $account_log = array(
        'user_id'       => $user_id,
        'user_money'    => $user_money,
...
function my_accountLog($user_id, $user_money = 0,
                       $pay_points = 0, $desc = '',$distribut_money = 0,
                       $order_id = 0 ,$order_sn = '',$address_id,$total_amount = 0,$jxmc=''){
...
//        " pay_points = pay_points + $pay_points, distribut_money = distribut_money + $distribut_money WHERE user_id = $user_id";
    ...
    $update_data = array(
        ...
        'pay_points'        => ['exp','pay_points+'.$pay_points],
        'distribut_money'   => ['exp','distribut_money+'.$distribut_money],
       // 'total_amount'   => ['exp','total_amount+'.$total_amount], //消费金额只用于维护购买人，此处不再维护 lishibo 20190529
...
function orderBtn($order_id = 0, $order = array())
{
    ...
    else
    {
        ...
        {
            //$btn_arr['return_btn'] = 1; // 退货按钮 (联系客服) //确认收货之后申请
           // $btn_arr['cancel_btn'] = 1; // 取消按钮 //update by lishibo 已支付或已确认订单不允许取消订单
        }
    ...
    //确认收货之后申请 update by lishibo
    if(in_array($order['order_status'], [2,4])){
        $btn_arr['return_btn'] = 1; // 退货按钮 (联系客服)
    }
    ...
    if($order['shipping_status'] == 2 && in_array($order['order_status'], [2,4])) // 部分发货
    {
        $btn_arr['return_btn'] = 1; // 退货按钮 (联系客服)
    }
 ...
 * @return bool|void
 */
function update_pay_statusSY($order_sn,$ext=array())
{
    if(stripos($order_sn,'recharge') !== false){
        ...
        if ($order['order_prom_type'] == 4) {
            ...
            /*2019-08-31
            //抢购间隔
            if($order['pay_name']=="批发券支付"){//如果是批发区购买 发货
                $dqqs['qianggou_time']=time();
                M('Users')->where(array('user_id'=>$order['user_id']))->update($dqqs);
            }
            //抢购间隔 end
            */
            //购买零售区商品  给推荐人5%奖金，更新推荐人的业绩
            if($order['pay_name']=="余额支付"){//是零售区
                $uu=M('users')->where("user_id=".$order['user_id'])->find();
                //增加购买人的新增业绩
                M('Users')->where("user_id",$order['user_id'])->setInc('yeji',$order['goods_price']);
                $jiexin=M('Users')->where("user_id",$order['user_id'])->setInc('xyeji',$order['goods_price']);
                if($jiexin){
                    $rebq['buy_user_id']=$order['user_id'];
                    $rebq['user_id']=$order['user_id'];
                    $rebq['nickname']=$uu['nickname'];
                    $rebq['order_sn']=$order_sn;
                    $rebq['order_id']=$order['order_id'];
                    $rebq['goods_price']=$order['goods_price'];
                    $rebq['money']=$order['goods_price'];
                    $rebq['status']=3;
                    $rebq['create_time']=time();
                    $rebq['jxmc']="购买零售区商品购买人获得新增业绩";
                    M('rebate_log')->add($rebq);
                    $logq['user_id']=$order['user_id'];
                    $logq['yeji']=$order['goods_price'];
                    $logq['change_time']=time();
                    $logq['desc']="购买零售区商品购买人获得新增业绩";
                    $logq['order_sn']=$order_sn;
                    $logq['order_id']=$order['order_id'];
                    $logq['jxmc']="购买人新增业绩";
                    M('account_log')->add($logq);
                }
                //增加购买人的新增业绩 end

                if($uu['first_leader']>0){//如果有上线   给推荐人5%奖金
                    $tui=M('users')->where("user_id=".$uu['first_leader'])->find();
                    //给上线增加新增业绩
                    while ($tui){
                        M('Users')->where("user_id",$tui['user_id'])->setInc('yeji',$order['goods_price']);
                        $jiexin=M('Users')->where("user_id",$tui['user_id'])->setInc('xyeji',$order['goods_price']);
                        if($jiexin){
                            $rebq['buy_user_id']=$order['user_id'];
                            $rebq['user_id']=$tui['user_id'];
                            $rebq['nickname']=$tui['nickname'];
                            $rebq['order_sn']=$order_sn;
                            $rebq['order_id']=$order['order_id'];
                            $rebq['goods_price']=$order['goods_price'];
                            $rebq['money']=$order['goods_price'];
                            $rebq['status']=3;
                            $rebq['create_time']=time();
                            $rebq['jxmc']="购买零售区商品上线获得新增业绩";
                            M('rebate_log')->add($rebq);
                            $logq['user_id']=$tui['user_id'];
                            $logq['yeji']=$order['goods_price'];
                            $logq['change_time']=time();
                            $logq['desc']="购买人（".$uu['user_id']."):".$uu['nickname']."购买零售区商品上线获得新增业绩";
                            $logq['order_sn']=$order_sn;
                            $logq['order_id']=$order['order_id'];
                            $logq['jxmc']="上线新增业绩";
                            M('account_log')->add($logq);
                    ...
                    //给上线增加新增业绩 end
                    $jiangjin=$order['goods_price']*0.05;
                    $ww=$tui['user_money']+$jiangjin;
                    $jie=M('Users')->where("user_id",$uu['first_leader'])->setInc('user_money',$ww);
                    if($jie){
                        $reb_log['user_id']=$uu['first_leader'];
                        $reb_log['buy_user_id']=$order['user_id'];
                        $reb_log['nickname']=$uu['nickname'];
                        $reb_log['order_sn']=$order_sn;
                        $reb_log['order_id']=$order['order_id'];
                        $reb_log['goods_price']=$order['goods_price'];
                        $reb_log['money']=$jiangjin;
                        $reb_log['level']=$tui['level'];
                        $reb_log['status']=3;
                        $reb_log['create_time']=time();
                        $reb_log['jxmc']="购买零售区商品直推人获得奖金";
                        M('rebate_log')->add($reb_log);
                        $log['user_id']=$uu['first_leader'];
                        $log['user_money']=$jiangjin;
                        $log['change_time']=time();
                        $log['desc']="购买零售区商品直推人获得奖金";
        ...
        /**************会员分成资格，决定是否分成的条件是产品属性，不再对级别权益进行判断 20190525************/
        $distribut_flag = false;
        ...
        foreach($order_goods as $k => $v) {
            ...
            /*购买成功以后获得批发卷，成为商城代理*/
            $pifahe=$pifahe+$goods['pifajuan']*$v['goods_num'];
            $total_amount=$total_amount+$goods['shop_price']*$v['goods_num'];


            $is_level = (int)$goods['is_level'];//商品按分类分组d
            if($is_level>0){
//                $distribut_flag = true;
            }
        }
...
if($pifahe>0){
    $shiling=2;//购买了零售区的商品
    $pfj_money=M('users')->where("user_id", $order['user_id'])->setInc('pfj_money', $pifahe);
    if($pfj_money){
        $account_logq = array(
        ...
        M('account_log')->add($account_logq);
    }

}
if($shiling==2){
    //维护消费金额
    $update_total_amount = array(
        'total_amount'   => ['exp','total_amount+'.$order['total_amount']], //消费金额只用于维护购买人
    );
    M('users')->where("user_id", $order['user_id'])->save($update_total_amount);
        ...
        // 减少对应商品的库存
        minus_stock($order['order_id'],$shiling);
 ...
 * @return array
 */
function confirm_order_copy($id,$user_id = 0){
    $where['order_id'] = $id;
    if($user_id){
        $where['user_id'] = $user_id;
    }
...
function calculate_price($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '')
{
    ...
    foreach ($order_goods as $key => $val) {
        ...
        //如果商品不是包邮的
        $isfreeshipping = (int)$goods_arr[$val['goods_id']]['is_free_shipping'];
        $goods_weight = 0;
        if ($isfreeshipping == 0){
...
function calculate_price_sanyu($user_id = 0, $order_goods, $shipping_code = '', $shipping_price = 0, $province = 0, $city = 0, $district = 0, $pay_points = 0, $user_money = 0, $coupon_id = 0, $couponCode = '')
{
    $couponLogic = new app\home\logic\CouponLogic();
    $goodsLogic = new app\home\logic\GoodsLogic();
    $user = M('users')->where("user_id", $user_id)->find();// 找出这个用户
