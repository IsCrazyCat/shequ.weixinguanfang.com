...
function getMenuList($act_list){
    ...
    if($act_list != 'all'){
        ...
        /********START！！ add by lishibo*  20190401*******/
        $role_right = array();
        foreach ($right as $val){
            $tmp_role_right = explode(',', $val);
            foreach ($tmp_role_right as $val_) {
                array_push($role_right,$val_);
            }
        }
...
function getMenuArr(){
    ...
    if($act_list != 'all' && !empty($act_list)){
        ...
        /********START！！权限功能动态可选 add by lishibo*  20190401*******/
        $role_right = array();
        foreach ($right as $val){
            $tmp_role_right = explode(',', $val);
            foreach ($tmp_role_right as $val_) {
        ...
        $role_right_ = implode(",", $role_right);

        foreach($menuArr as $k=>$val){
            foreach ($val['child'] as $j=>$v){
                foreach ($v['child'] as $s=>$son){
                    if(strpos($role_right_,$son['op'].'Controller@'.$son['act']) === false){
...
function getMenuList_bak($act_list){
	//根据角色权限过滤菜单
	$menu_list = getAllMenu();
	if($act_list != 'all'){
		$right = M('system_menu')->where("id", "in", $act_list)->cache(true)->getField('right',true);
