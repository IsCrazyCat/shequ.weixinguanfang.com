...
class Goods extends Base {
    /*批发商品冷冻时间*/
    public function lengdong(){
        if(IS_POST){
            $shijuanduan= I('endTime');

            $find=M('config')->where(array('name'=>"lengdong"))->find();
            if($find){
                $data['value']=$shijuanduan;
                $sh=M('config')->where(array('name'=>"lengdong"))->update($data);
                if($sh){
                    $this->success('时间段设置成功');
                }else{
                    $this->error('时间段设置失败');
                }
            }else{
        ...
        $find=M('config')->where(array('name'=>"lengdong"))->find();
        $this->assign('endTime',$find['value']);
        return $this->fetch();
    }
/*批发区购买定时*/
    public function dingshi(){
        if(IS_POST){
            $shijuanduan= I('dingshi');//小时段
            $test11= I('test11');//日期
            $jianshu= I('jianshu');//抢购件数
            $jiange= I('jiange');//抢购间隔
            //抢购间隔
            $finge=M('config')->where(array('name'=>"jiange"))->find();
            if($finge){
                $datagg['value']=$jiange;
                M('config')->where(array('name'=>"jiange"))->update($datagg);
            }else{
                $data2g['value']=$jiange;
                $data2g['name']="jiange";
                M('config')->add($data2g);
            ...
            //抢购间隔  end
            //抢购件数
            $find2=M('config')->where(array('name'=>"jianshu"))->find();
            if($find2){
                $data['value']=$jianshu;
                $countq=M('config')->where(array('name'=>"jianshu"))->update($data);
                    $count['store_count']=$jianshu;
                   M('goods')->where(array('goods_id'=>32))->update($count);
            }else{
                $data2['value']=$jianshu;
                $data2['name']="jianshu";
                M('config')->add($data2);
            ...
            //日期
            $find5=M('config')->where(array('name'=>"test11"))->find();
            if($find5){
                $data5['value']=$test11;
                $sh=M('config')->where(array('name'=>"test11"))->update($data5);

            }else{
                $data5['value']=$test11;
                $data5['name']="test11";
                $add=M('config')->add($data5);

            }
            //小时段
            $find=M('config')->where(array('name'=>"dingshi"))->find();
            if($find){
                $data['value']=$shijuanduan;
                $sh=M('config')->where(array('name'=>"dingshi"))->update($data);
                if($sh){
                    $this->success('提交成功');
                }else{
                    $this->error('提交成功');
        ...
        $jianshu=M('config')->where(array('name'=>"jianshu"))->find();
        //抢购间隔
        $jiange=M('config')->where(array('name'=>"jiange"))->find();
        $this->assign('jiange',$jiange['value']);//间隔时间
        //抢购间隔  end
        $this->assign('dingshi',$find['value']);//小时段
        $this->assign('test11',$test11['value']);//日期
        $this->assign('jianshu',$jianshu['value']);//件数
        return $this->fetch();
    }
     ...
     * @return mixed
     */
    public function virtualComments()
    {
        if (IS_POST) {
            // 晒图片
...
//            $add['email'] = $user_info['email'];
            $add['username'] = '匿名';
            $add['is_anonymous'] = 1;  //是否匿名评价:0不是\1是
...
/*            $add['service_rank'] = I('service_rank');
            ...
            //添加评论
            $row = $logic->add_comment_virtualComments($add);
            if ($row['status'] == 1) {
                $this->success('评论成功', U('Admin/Goods/goodsList', array('status'=>1)));
                exit();
            } else {
                $this->error($row['msg']);
            }
        ...
        $rec_id = I('id/d');
        $goods = M('goods')->where("goods_id", $rec_id)->find();
