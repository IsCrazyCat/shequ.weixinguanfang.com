...
/*转卖列表*/
    ...
    public function ajaxzhuanmai(){
        ...
        $name = ($keyType && $keyType == 'nickname') ? $keywords : I('nickname') ;
        $name ? $condition['name'] = trim($name) : false;
...
if(empty($condition) && $keyType!="gmz" && $keyType!="ymc"){
    $count = M('daishou_goods')->where($condition)->count();
}elseif(!empty($name)) {
    $nau=M('users')->where(array('name' =>$name))->find();
    $count = M('daishou_goods')->where(array('user_id'=>$nau['user_id']))->count();
}elseif(!empty($mobile)){
    $nau=M('users')->where(array('nickname' =>$mobile))->find();
    $count = M('daishou_goods')->where(array('user_id'=>$nau['user_id']))->count();
}elseif ($keyType=="gmz"){//挂卖中。。
    $count = M('daishou_goods')->where(array('status'=>0))->count();
}else{
    $count = M('daishou_goods')->where(array('status'=>1))->count();
}

        $Page  = new AjaxPage($count,12);
        if(empty($condition) && $keyType!="gmz" && $keyType!="ymc"){
            $userList = M('daishou_goods')
                ->where($condition)
                ->order($sort_order)
            ...
            $nau=M('users')->where(array('name' =>$name))->find();
            $userList = M('daishou_goods')
                ->where(array('user_id'=>$nau['user_id']))
                ->order($sort_order)
                ->limit($Page->firstRow.','.$Page->listRows)
                ->select();
        }elseif(!empty($mobile)){
        ...
        foreach ($userList as& $val){
            $userq=M('users')->where("user_id=".$val['user_id'])->find();
            $val['zmrzh']=$userq['nickname'];
            $val['name']=$userq['name'];
        }
