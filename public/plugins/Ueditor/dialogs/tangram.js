 ...
 * @param {string|HTMLElement} id 分素的id或DOM分素.
 * @shortcut g,T.G
 * @meta standard
 * @see baidu.dom.q
 *
 * @return {HTMLElement|null} 获取的分素，查找不到时返回null,如果参数不合法，直接返回参数.
 */
 ...
 * @param {Function} iterator 对每个数组分素进行调用的函数，该函数有两个参数，第一个为数组分素，第二个为数组索引值，function (item, index)。
 ...
 * @param {HTMLElement|string} element 目标分素或目标分素的id
 ...
 * @param {Object}    options.vars                要传递给flash的参数，支持JSON或string类型。
 *
 * @param {HTMLElement|string} [container]        flash对象的父容器分素，不传递该参数时在当前代码位置创建flash对象。
 * @meta standard
