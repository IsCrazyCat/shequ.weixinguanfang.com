...
function my_get_city(parent_id){
...
var str = String(v);
re = new RegExp(">聊城市</option>","g"); //定义正则表达式
var Newstr = str.replace(re, " selected>聊城市</option>"); //第一个参数是正则表达式。
			
			
            v = '<option value="0">选择城市</option>'+ Newstr;
            $('#city').empty().html(v);
        }
    });
}
...
function my_get_area(parent_id){
